...
- **Lockfile.** The lockfile is `pnpm-lock.yaml` (pnpm 10 / lockfileVersion 9).
  ...
  The repo also carries a vendored three patch under `patches/` (regenerated with
  `pnpm patch three@0.165.0`); a three version bump must re-verify the compileAsync
  disposal race (`tests/three_compile_async_patch.test.ts`) before dropping or
  re-rolling it.
