...
describe('desktop runtime helpers', () => {
  ...
  it('normalizes HTTP origins and rejects non-web origins', () => {
    expect(normalizeOrigin('https://wildhaven.example/')).toBe(
      'https://wildhaven.example',
    );
    expect(() => normalizeOrigin('app://wildhaven')).toThrow(
  ...
  it('builds websocket URLs from desktop API origins', () => {
    expect(
      runtimeWebSocketUrl('app:', 'wildhaven', 'https://wildhaven.example'),
    ).toBe('wss://wildhaven.example/ws');
