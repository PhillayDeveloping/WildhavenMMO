...
describe('PgDailyRewardDb.unannouncedWinnerDays (the announcement-narrow read)', () => {
  ...
  it('maps a WIDE database row down to exactly the six announced fields', async () => {
    ...
    // The decisive absence pin: key EQUALITY, so a mapper that widened back out
    // to the whole row (whose extra fields are structurally assignable) fails
    // here even though it would type-check. In this fork that pin also carries
    // the web3 removal: the fixture row above deliberately still holds
    // wallet_pubkey, tx_signature and signed_transaction, so if any of them ever
    // reaches the announcement payload again, this equality is what says so.
