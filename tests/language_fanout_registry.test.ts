...
describe('language fan-out: half 2, every signature-gated src/ui surface is classified', () => {
  ...
  it('holds every row to a written reason', () => {
    ...
    expect(
      ...
      // 5 = upstream's six minus the one the web3 removal took away.
      // Upstream counts guild_bank_window's `prevReadOnly` (announcement-only:
      // it decides whether the read-only note is a live region on the
      // demotion-edge paint, never what is drawn), which this fork keeps.
      // What this fork does NOT have is claudium_window.ts's row for
      // `paintedWalletMarkup`: the wallet connection panel that memo elided no
      // longer exists, so the module carries no compared memo at all. The count
      // going down because a module lost its memo is the healthy direction;
      // this number exists to make growth deliberate.
    ).toBe(5);
  });
