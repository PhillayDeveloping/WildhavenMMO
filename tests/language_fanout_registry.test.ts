...
describe('language fan-out: half 2, every signature-gated src/ui surface is classified', () => {
  ...
  it('holds every row to a written reason', () => {
    ...
    expect(
      ...
      // switch.
      // 7 here, not upstream's 8: what this fork does NOT have is
      // claudium_window.ts's row for `paintedWalletMarkup`, since the wallet
      // connection panel that memo elided no longer exists, so the module
      // carries no compared memo at all. The count going down because a module
      // lost its memo is the healthy direction; this number exists to make
      // growth deliberate.
    ).toBe(7);
  });
