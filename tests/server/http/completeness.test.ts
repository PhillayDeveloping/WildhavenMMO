...
describe('registry completeness: oauth + internal surfaces (server/oauth.ts, server/internal.ts, server/daily_rewards.ts)', () => {
  ...
  it('derives the expected non-empty ladders', () => {
    ...
    // 17 = the handleInternalApi nine (restart-countdown + the 8 Discord-bot
    // routes) plus this fork's six-route ops family plus the two registry-only
    // rows. One below upstream's 18: their seventh ops route is the payout
    // runner's mark, which has no counterpart here (the purse is in-game coin
    // delivered by Ravenpost, so there is no external payment to stamp).
