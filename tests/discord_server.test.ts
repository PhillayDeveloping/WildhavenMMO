...
function makeReq(opts: { url?: string; body?: unknown; origin?: string } = {}): any {
  ...
  req.url = opts.url ?? '/';
  req.headers = { host: 'wildhaven.example', ...(opts.origin ? { origin: opts.origin } : {}) };
...
const NATIVE_STATE_REDIRECT = `wildhaven://discord-auth?challenge=${NATIVE_CHALLENGE}`;
...
describe('GET /api/auth/discord/callback', () => {
  ...
  it('returns a native returning user through a one-time handoff code', async () => {
    ...
    expect(location.protocol).toBe('wildhaven:');
