...
// Namespace import on ONE line, deliberately: a named-import list long enough to
// wrap puts the ts-expect-error on `import {` instead of the module specifier that
// actually errors, and the suppression silently stops working.
// @ts-expect-error - shared zero-dep JS tool (no .d.ts); same pattern as the registry test importing scripts/i18n_hash.mjs.
import * as worklist from '../scripts/i18n_fill_worklist.mjs';
...
describe('worklist assembly (deterministic + blocked-prose segregation, end to end)', () => {
  ...
  const glossarySrc = {
    verbatim: [{ term: 'Wildhaven', note: 'brand' }],
  ...
  it('ships the glossary in every batch and resolves each term per-locale (own overlay -> English)', () => {
    ...
    const de = batchObjs.get('de_DE');
    expect(de.glossary.verbatim.map((v: any) => v.term)).toContain('Wildhaven');
