...
describe('resolveWikiUrl', () => {
  it('stays same-origin on any http(s) deploy, so a dev deploy links its own wiki', () => {
    expect(resolveWikiUrl({ nativeApp: false, origin: 'https://dev.wildhaven.example' })).toBe(
      'https://dev.wildhaven.example/wiki/',
    );
    expect(resolveWikiUrl({ nativeApp: false, origin: 'https://wildhaven.example' })).toBe(
      'https://wildhaven.example/wiki/',
    );
  ...
  it('pins the canonical wiki URL to its literal', () => {
    expect(CANONICAL_WIKI_URL).toBe('https://wildhaven.example/wiki/');
  });
});

describe('promptWikiVisit', () => {
  it('asks with the wiki dialog copy and opens the resolved URL only on OK', () => {
    vi.stubGlobal('location', { origin: 'https://dev.wildhaven.example' });
    const openUrl = vi.fn();
    const confirm = vi.fn();
    ...
    expect(body).toBe('This opens the Wildhaven wiki in your browser. The game keeps running.');
    ...
    expect(openUrl).toHaveBeenCalledWith('https://dev.wildhaven.example/wiki/');
  });
