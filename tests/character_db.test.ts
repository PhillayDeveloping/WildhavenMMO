...
describe('bankBonusFactsForAccount', () => {
  ...
  // The wallet source is gone with the rest of web3, which is why this is three
  // facts and not four; BANK_MAX_BONUS_SLOTS dropped in lockstep.
  it('reads all three facts in one parameterized query carrying the load-bearing predicates', async () => {
    ...
    // The wallet source is gone, not merely unused: a query that still joined
    // wallet_links would mean the removal missed the read path.
    expect(sql).not.toMatch(/wallet/i);
    ...
    // Invariant: no balance/holder-tier/chain read survives anywhere in the query.
