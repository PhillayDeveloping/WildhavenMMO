  ...
  it('selects the matching trusted realm origin', () => {
    expect(
      publicOriginForRealm('Ironforge', [
        { name: 'Wildmoon', url: 'https://wildmoon.example.com', type: 'Normal' },
...
describe('GET /p/<slug>', () => {
  ...
  it('uses the trusted dev host in production mode instead of the production fallback', async () => {
    await withReloadedCardRoutes({ NODE_ENV: 'production' }, async (routes) => {
      ...
      await routes(
        makeGetReq('/p/sir-test', {
          headers: { host: 'dev.wildhaven.example', 'x-forwarded-proto': 'https' },
        }),
        res,
      );
      const html = String(res.body);
      expect(res.statusCode).toBe(200);
      expect(html).toContain(
        '<link rel="canonical" href="https://dev.wildhaven.example/p/sir-test">',
      );
      expect(html).toContain(
        'property="og:url" content="https://dev.wildhaven.example/p/sir-test"',
      );
      expect(html).toContain(
        'property="og:image" content="https://dev.wildhaven.example/p/sir-test/card.png"',
      );
      expect(html).toContain('src="/p/sir-test/card.png"');
      expect(html).toContain('href="/?ref=sir-test"');
    });
    ...
    await withReloadedCardRoutes(
      {
        ...
        REALMS:
          'Wildmoon=https://wildmoon.example.com=Normal,Ironforge=https://ironforge.example.com=PvP',
      },
      async (routes) => {
        cardRows = [
          { character_id: 5, account_id: 1, png: validCardPng, title: 't', description: 'd' },
