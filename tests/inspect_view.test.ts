...
describe('buildInspectView: the Curator sigil badge (rank 5 only)', () => {
  ...
  it('keeps the two older badge slots untouched at every Curator rank', () => {
    // The sigil is a THIRD slot, not a replacement: a rank-5 player with no
    // other flair still shows exactly one badge. (Upstream counts a fourth,
    // the $WOC holder badge; this fork ships no wallet, so the slot is absent.)
    const m = buildInspectView(ranked(5), ITEMS).badges;
    expect(m.discord).toBeNull();
    expect(m.dev).toBeNull();
    expect(Object.keys(m).sort()).toEqual(['curator', 'dev', 'discord']);
  });
});
