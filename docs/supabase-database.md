...
If the connection fails TLS verification rather than authentication, the causes
worth checking in order are: a missing intermediate in your system trust store,
a corporate TLS-inspecting proxy, and only then the certificate itself.
