...
const MAC_DMG_RE = /wildhaven-\d+\.\d+\.\d+-mac-universal\.dmg/g;
const LINUX_APPIMAGE_RE = /wildhaven-\d+\.\d+\.\d+-linux-x86_64\.AppImage/g;
...
const WINDOWS_INSTALLER_RE = /wildhaven-\d+\.\d+\.\d+-win-x64\.exe/g;
...
const LEGACY_WINDOWS_INSTALLER_RE = /wildhaven-\d+\.\d+\.\d+-win\.exe/g;
...
export function setDesktopDownloadVersion(html, version, path) {
  ...
  return html
    .replace(MAC_DMG_RE, `wildhaven-${normalized}-mac-universal.dmg`)
    .replace(LINUX_APPIMAGE_RE, `wildhaven-${normalized}-linux-x86_64.AppImage`)
    .replace(WINDOWS_INSTALLER_RE, `wildhaven-${normalized}-win-x64.exe`)
    .replace(LEGACY_WINDOWS_INSTALLER_RE, `wildhaven-${normalized}-win-x64.exe`);
}
...
export function collectReleaseVersionFailures({
  ...
  const expectedArtifact = `wildhaven-${expected}-mac-universal.dmg`;
  const expectedLinuxArtifact = `wildhaven-${expected}-linux-x86_64.AppImage`;
  const expectedWindowsArtifact = `wildhaven-${expected}-win-x64.exe`;
