...
if [ "${seed_hash}" != "${current_hash}" ]; then
  echo "Importing Wildhaven wiki seed pages..."
  printf '%s\n' 'Main Page' | php maintenance/deleteBatch.php --u "${MEDIAWIKI_ADMIN_USER}" --r 'Replace stock install page with Wildhaven seed' || true
