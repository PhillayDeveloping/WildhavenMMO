...
// client-side: class, skin, worn gear, the Discord / dev identity flair, and the
// server-computed Curator standing) into the structured model the painter draws:
// a compact header (name, deed title, level, class, class color, Book of Deeds
// border accent), the three badge decisions (Discord / dev / Curator sigil, each
// gated exactly as the old inline card gated them), the Reliquary standing line,
// and the worn-gear paperdoll (reused from char_view's buildPaperdollView, so
// inspect inherits the sheet's 6/6 column split). A separate builder covers the
// thinner out-of-range remote-profile card.
//
...
export function buildInspectView(
  ...
  return {
    header,
    badges: { discord, dev, curator: curatorBadge },
    curator,
