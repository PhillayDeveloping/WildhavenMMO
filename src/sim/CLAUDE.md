...
| `mail/post_office.ts` | player mail (send/take/read/delete, the mailbox anchor gate); every read rides the per-recipient `MailIndex` buckets, every observable mutation advances the book revision the server's `mail` snapshot gate polls (`mailRevFor`, null away from a pillar). The book itself is PRIVATE behind a readonly `mail` getter: since the reads moved onto the buckets, a letter added without a matching `index.track` is INVISIBLE (untakeable, undeletable, and `mailboxHoldsItem` false, which mints a duplicate quest item on re-accept), so `book()`/`loadMail` are the only writers and an outside push is a type error. Pinned by `expectMailBookMatchesIndex` (`tests/helpers/mail_book_invariant.ts`) |
