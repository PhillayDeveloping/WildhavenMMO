...
- Economy cluster (`stripe_checkout.ts`, `economy_sdk.ts`, `discord_onboarding_gate.ts`):
  the CLAUDIUM economy client. This fork ships no wallet and no chain linking, so the
  upstream `wallet*.ts` / `desktop_wallet_*.ts` / `mobile_wallet_deeplink.ts` /
  `seeker_entitlement_sync.ts` family is deliberately absent here
  (`tests/no_web3_regression.test.ts` fails if any of it returns through a sync). The
  contracts that remain: nothing here is imported by `src/sim/`, and `economy_sdk.ts` is
  same-origin only (the game server's `/api/claudium/*` proxy, never the economy service)
  and NEVER throws into render: every failure resolves to the typed unavailable state the
  disabled UI already renders. `discord_onboarding_gate.ts` is the shared predicate that
