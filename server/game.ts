...
export class GameServer {
  ...
  //
  // Unlike the two flair refreshers beside it this is pure CPU off the LIVE sim
  // meta (one catalog walk, no DB row, no HTTP), so it is synchronous and needs no
  ...
  // below belongs to the two AWAITED refreshers (Discord, GitHub): one of those
  // cycles can outrun the interval and must not pile up. The curator sweep is
  // pure CPU off live sim meta with no IO of its own, so it can never be the
  // thing that piles up, and leaving it under the guard meant one degraded
  // fetch cycle froze every online player's Curator standing for as long as that
  // cycle hung. The per-session try/catch is the same best-effort contract the
  // .catch arms give the awaited pair: one throwing session must not kill the
